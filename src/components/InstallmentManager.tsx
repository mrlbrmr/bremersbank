...
const InstallmentManager = () => {
  ...
  const handleAdvance = async (inst: Installment) => {
    ...
    toast.success(`Parcela ${next}/${inst.total_installments} marcada!`);
    fetchInstallments();
  };
          ...
          return (
            <div key={inst.id} className="rounded-xl border border-border bg-card p-4 shadow-sm hover:shadow-md transition-all">
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span className="font-medium text-foreground">{inst.current_installment}/{inst.total_installments} parcelas</span>
